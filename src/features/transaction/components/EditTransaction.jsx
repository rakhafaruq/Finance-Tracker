...
export default function EditPage() {
    ...
    const { editTransaction, transaction } = useFinance();
    ...
    useEffect(() => {
        ...
        const existingId = transaction.find((t) => t.id === txId);

        if (existingId) {
            setValue("title", existingId.title);
            setValue("amount", existingId.amount);
            setValue("type", existingId.type);
            setValue("category", existingId.category);
            setValue("date", existingId.date);
            setLoading(false);
        } else if (transaction.length > 0) {
            alert("Transaction Not Found..");
            router.push("/transaction");
        }
    }, [params.id, setValue, transaction, router]);
    ...
    const onSubmit = (data) => {
        ...
        setIsSubmit(false);
        editTransaction(Number(params.id), updateTx);
        router.push("/transaction");
    };
        ...
        <div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="pt-4 flex justify-between gap-4">
                    ...
                    <button onClick={() => router.back()} type="submit" className="text-gray-700 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer p-2 w-full text-sm transition-all border border-gray-200">
                        Cancel
                    </button>
                </div>
            </form>
